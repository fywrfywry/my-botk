...
# إعداد التسجيل لمتابعة أداء السكريبت
logging.basicConfig(
...
# تشغيل العميل بصلاحيات البوت
client = TelegramClient('bot_session', API_ID, API_HASH)

def is_luhn_valid(card_number: str) -> bool:
    """خوارزمية Luhn للتحقق من صحة البطاقات رياضياً"""
...
async def process_ulp_file_with_dashboard(input_path: str, output_path: str, status_msg) -> int:
    """معالجة وفلترة الملف الكبير واستخراج البطاقات مع تحديث اللوحة"""
            ...
            if line_no % 250000 == 0:
                try:
                    await status_msg.edit(
                        ...
                        f"----------------------------------\n"
                        f"🔄 **الحالة:** جاري فحص الأسطر...\n"
                        f"📝 **المفحوصة:** {line_no:,}\n"
                        f"💳 **الصالحة:** {len(found_entries):,}\n"
                        f"----------------------------------"
                    )
                except Exception:
                    pass

            for match in card_full_pattern.findall(line):
                raw_card, raw_exp, cvv = match
                clean_card = re.sub(r'\D', '', raw_card)
                exp_digits = re.findall(r'\d+', raw_exp)
                if len(exp_digits) == 2:
                    formatted_exp = f"{exp_digits[0].zfill(2)}|{exp_digits[1]}"
                else:
                    continue

                if 13 <= len(clean_card) <= 19 and is_luhn_valid(clean_card):
                    found_entries.add(f"{clean_card}|{formatted_exp}|{cvv}")
...
# مستقبل الرسائل التلقائي للروابط
@client.on(events.NewMessage(incoming=True))
async def handle_all_messages(event):
    message_text = event.raw_text or ""
    
    # التحقق من وجود رابط تيليجرام في الرسالة
    match = re.search(r't\.me/(?:c/)?([^/]+)/(\d+)', message_text)
    if not match:
        return
    ...
    msg = await event.respond(
        ...
        "----------------------------------\n"
        "⏳ **الحالة:** تم استلام الرابط، جارٍ الاتصال وسحب الملف...\n"
        "----------------------------------"
    )
    ...
    try:
        ...
        if not target_msg or not target_msg.media:
            await msg.edit("❌ الرابط المرسل لا يحتوي على ملف أو أن الوصول إليه متعذر.")
            return
        ...
        if cards_count > 0:
            ...
            await client.send_file(
                event.chat_id,
                output_path,
                caption=f"📄 تم استخراج {cards_count:,} بطاقة نظيفة بنجاح."
            )
        else:
...
if __name__ == '__main__':
    print("السكريبت يعمل الآن وجاهز لاستقبال الروابط...")
